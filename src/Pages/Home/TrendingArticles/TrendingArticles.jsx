...
const TrendingSlider = () => {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      try {
        const response = await axios.get('http://localhost:9000/articles');  // Replace with your actual API endpoint
    ...
    >
      <div className="relative w-20 h-20 flex-shrink-0">
        ...
        <div className={`absolute inset-0 rounded-lg transition-opacity ${
          isActive ? 'opacity-25 bg-red-500' : 'opacity-0 group-hover:opacity-10 bg-black'
        }`} />
      </div>
      <div className="flex-1 min-w-0">
        <h5 className={`font-semibold text-sm line-clamp-2 mb-1 transition-colors ${
          isActive ? 'text-red-900' : 'group-hover:text-red-700'
        }`}>
          {article.title}
        </h5>
      ...
      <ChevronRight className={`w-5 h-5 flex-shrink-0 transition-all ${
        isActive ? 'text-red-500 opacity-100' : 'opacity-0 group-hover:opacity-50 -translate-x-2 group-hover:translate-x-0'
      }`} />
    </div>
  );
